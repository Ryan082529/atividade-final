<!-- src/components/FormularioUnificado.vue -->
<template>
  <form class="form" @submit.prevent="enviar">
    <!-- Campos para mensagem (título e autor) -->
    <template v-if="tipo === 'mensagem'">
      <div class="grupo">
        <label for="titulo">Título</label>
        <input 
          id="titulo"
          v-model.trim="formulario.titulo"
          type="text"
...
// Preencher autor com o usuário logado em modo criação
watch(() => props.autorPadrao, (novo) => {
  if (props.tipo === 'mensagem' && props.modo === 'criacao') {
    formulario.value.autor = novo || ''
  }
}, { immediate: true })
...
function enviar() {
  ...
  // Adicionar título apenas para mensagens
  if (props.tipo === 'mensagem') {
    dados.titulo = formulario.value.titulo
  }
